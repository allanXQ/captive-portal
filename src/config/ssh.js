...
class RouterSSHClient {
  ...
  async handleDisconnection() {
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      console.log(`Attempting to reconnect to SSH (attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts})...`);
      ...
      console.error('Maximum reconnection attempts reached. SSH connection lost permanently.');
    }
  }
  ...
  async executeCommand(command) {
    ...
    while (retries > 0) {
      try {
        ...
        if (retries > 0) {
          console.log(`Retrying command execution... (${retries} attempts left)`);
          await new Promise(resolve => setTimeout(resolve, 1000)); // Wait 1 second before retry
        } else {
          console.error(`Command execution failed after all retries: ${command}`);
          throw error;
        }
      }
    }
  ...
  async authenticateUser(macAddress) {
    try {
      ...
      console.error(`Failed to authenticate user ${macAddress}:`, error.message);
      throw error;
    }
  }
  ...
  async deauthenticateUser(macAddress) {
    try {
      ...
      console.error(`Failed to deauthenticate user ${macAddress}:`, error.message);
      throw error;
    }
  }
